...
class TagController extends AbstractController {
     ...
     */
    public function index(TagsMapper $tagsMapper, Request $request, $page=1, $perPage=20, $area=0, $sort=0, $sortDESC=0) {
    ...
    private function parseTagError($errorObj, Form $form) {
        ...
        if ($errorObj instanceof AppException) {
            ...
            switch ($code) {
                ...
                default: $form->get('tName')->addError(new FormError('Unknown exception!'));
            }
    ...
    private function prepareTagTypes(array $tags): array {
        ...
        if (count($tags)==1) {
            ...
            if ($tags[0]->getType()->getId() == TagType::Bit) {
                $type = 'Bit';
            } else if ($tags[0]->getType()->getId() == TagType::REAL) {
                $type = 'Real';
            } else {
                $type = 'Numeric';
            }
