...
class ScriptItemController extends AbstractController {
     ...
     */
    public function index(ScriptItemMapper $scriptMapper, Request $request, $page=1, $perPage=20, $area=0, $sort=0, $sortDESC=0) {
    ...
    private function parseScriptItemError($errorObj, Form $form) {
        ...
        if ($errorObj instanceof AppException) {
            ...
            } else if ($code == AppException::SCRIPT_FILE_EXIST || $code == AppException::SCRIPT_FILE_NOT_EXIST) {
     ...
     */
    public function add(ScriptItemMapper $scriptMapper, TagsMapper $tagsMapper, Request $request, ConfigGeneralMapper $cfg) {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            try {
                ...
                if (!file_exists(RunScriptCommand::buildScriptPath($cfg->getUserScriptsPath(), $script->getName()))) {
                    throw new AppException("Script: ".$script->getName()." does not exist on disk!", AppException::SCRIPT_FILE_NOT_EXIST);
                }
     ...
     */
    public function edit($scriptID, ScriptItemMapper $scriptMapper, TagsMapper $tagsMapper, Request $request, ConfigGeneralMapper $cfg) {
