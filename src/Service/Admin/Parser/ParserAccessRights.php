            ...
            if (!$this->security->isGranted('ROLE_ADMIN')) {
                ...
                if ($accessRightItem['role'] != 'ROLE_GUEST') {
                    ...
                    if (!$this->security->isGranted($accessRightItem['role'])) {
                        throw new AppException('Calling '.$accessRightItem['specialFunc'].' is forbidden', AppException::SPECIAL_FUNCTION_DENIED);
                    }
                }
            }
            ...
            if ($accessRightItem['read']) {
                ...
                if ($tag->getReadAccess() != 'ROLE_GUEST') {
                    ...
                    if (!$this->security->isGranted($tag->getReadAccess())) {
                        throw new AppException('Reading from '.$tag->getName().' is forbidden', AppException::TAG_READ_ACCESS_DENIED);
                    }
                }
                ...
                if ($tag->getWriteAccess() != 'ROLE_GUEST') {
                    ...
                    if (!$this->security->isGranted($tag->getWriteAccess())) {
                        throw new AppException('Writting to '.$tag->getName().' is forbidden', AppException::TAG_WRITE_ACCESS_DENIED);
                    }
                }
            }
        }
